...
def main():
    ...
    # svm = SVMTrainingModule(trainSet, "test",PRE_PROCES_PARAMS,FFT_PARAMS, modelName = "test")
    ...
    modelo = svm.createSVM(kernel = "rbf", gamma = 0.05, C = 24, probability = True)
    svm.model

    filteredEEG = filterEEG(svm.rawDATA, svm.PRE_PROCES_PARAMS["lfrec"],
    ...
    dataBanked = svm.applyFilterBank(filteredEEG, bw=2.0, order = 4)

    anchoVentana = int(fm*5) #fm * segundos
    ventana = windows.hamming(anchoVentana)

    sampleFrec, signalPSD = svm.computWelchPSD(dataBanked,
                            fm = svm.PRE_PROCES_PARAMS["sampling_rate"],
                            ventana = ventana, anchoVentana = anchoVentana, average = "median", axis = 1)

    trainingData, labels = svm.getDataForTraining(features = signalPSD)

    metricas = svm.trainAndValidateSVM(clases = np.arange(0,len(frecStimulus)), test_size = 0.2)
    print(metricas)
