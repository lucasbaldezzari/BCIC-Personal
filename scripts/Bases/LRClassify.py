...
EEGS8 = rawEEGs["s8"]["eeg"][:, :, :, :11]

#Selecting the last 3 trials for testing
testEEGS8 = rawEEGs["s8"]["eeg"][:, :, :, 11:] #testing set
...
trainSetEEG8Partitions = list(partitionsGenerator.split(trainSetEEGS8, train_labelsEEGS8))
...
for clase in np.arange(0,12):
    ...
    for train_ind, test_ind in trainSetEEG8Partitions: 
        ...
        model.fit(trainSetEEGS8[train_ind, :], train_labelsEEGS8[train_ind, clase]) #entreno modelo
    
        pred = model.predict(trainSetEEGS8[test_ind, :])
        if accuracy_score(train_labelsEEGS8[test_ind,:][:,clase], pred) > accu:
            
            accu = accuracy_score(train_labelsEEGS8[test_ind,:][:,clase], pred)
...
clase = 1
canal = 1
numberFeatures = testEEGS8MagFFT.shape[0]
trials = testEEGS8MagFFT.shape[3]
data = testEEGS8MagFFT[:, canal-1, clase - 1, :, :].reshape(numberFeatures,trials).swapaxes(0,1)
